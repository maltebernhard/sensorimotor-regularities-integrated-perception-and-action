...
class Triangulation_AI(ActiveInterconnection):
    def __init__(self, estimators: List[RecursiveEstimator], device, object_name:str="Target") -> None:
        ...
        required_estimators = [f'Polar{object_name}PosRadius', 'RobotVel']
    ...
    def implicit_interconnection_model(self, meas_dict: Dict[str, torch.Tensor]):
        ...
        offset_angle = meas_dict[f'Polar{self.object_name}PosRadius'][1]
        ...
        angular_vel = meas_dict[f'Polar{self.object_name}PosRadius'][3] + meas_dict['RobotVel'][2]
        if torch.abs(robot_target_frame_vel[1]) == 0.0 or torch.abs(angular_vel) == 0.0:
            triangulated_distance = meas_dict[f'Polar{self.object_name}PosRadius'][0]
        else:
            # TODO: tan or plain?
        ...
        return torch.stack([
            torch.atleast_1d(triangulated_distance - meas_dict[f'Polar{self.object_name}PosRadius'][0]),
            torch.atleast_1d(- robot_target_frame_vel[0] - meas_dict[f'Polar{self.object_name}PosRadius'][2]),
        ]).squeeze()