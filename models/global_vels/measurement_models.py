...
class Angle_MM(ActiveInterconnection):
    ...
    def implicit_interconnection_model(self, meas_dict: Dict[str, torch.Tensor]):
        return torch.stack([
            ...
            meas_dict[f'{self.object_name.lower()}_offset_angle_dot'] - (meas_dict[f'Polar{self.object_name}Pos'][3] + meas_dict[f'Polar{self.object_name}Pos'][5]),
            meas_dict[f'{self.object_name.lower()}_visual_angle']     - meas_dict[f'Polar{self.object_name}Pos'][6],
            meas_dict[f'{self.object_name.lower()}_visual_angle_dot'] - meas_dict[f'Polar{self.object_name}Pos'][7],
        ]).squeeze()