...
class Foveal_Angle_AI(ActiveInterconnection):
    def __init__(self, object_name:str="Target") -> None:
        self.object_name = object_name
        super().__init__([f'Polar{object_name}Pos', f'{object_name}FovealVision'])

    def implicit_interconnection_model(self, meas_dict: Dict[str, torch.Tensor]):
        return torch.stack([
            (meas_dict[f'{self.object_name}FovealVision'][0] - meas_dict[f'Polar{self.object_name}Pos'][1] + torch.pi) % (2*torch.pi) - torch.pi,
            meas_dict[f'{self.object_name}FovealVision'][1] - meas_dict[f'Polar{self.object_name}Pos'][3],
        ]).squeeze()