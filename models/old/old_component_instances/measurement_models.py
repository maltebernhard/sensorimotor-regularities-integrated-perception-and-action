...
class Pos_Angle_MM(ActiveInterconnection):
    """
    Measurement Model:
    ...
    def implicit_interconnection_model(self, meas_dict):
        ...
        if meas_dict[f"Cartesian{self.object_name}Pos"][1] == 0.0 and meas_dict[f"Cartesian{self.object_name}Pos"][0] == 0.0:
        ...
        return torch.concat([
            ...
            - torch.concat([meas_dict['vel_frontal'], meas_dict['vel_lateral'], meas_dict['vel_rot']]) - meas_dict[f"Cartesian{self.object_name}Pos"][2:],
        ]).squeeze()
...
class Robot_Vel_MM(ActiveInterconnection):
    ...
    def implicit_measurement_model(self, meas_dict):
        return torch.stack([
