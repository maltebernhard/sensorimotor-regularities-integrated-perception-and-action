...
class PolarGoToTargetGazeFixationGoal(Goal):
    ...
    def loss_function(self, buffer_dict: Dict[str, Dict[str, torch.Tensor]]):
        loss_mean = torch.concat([
            ...
            1e0 * torch.atleast_1d(buffer_dict['PolarTargetPos']['mean'][0] - self.desired_distance),
            # GazeFixation
            1e1 * torch.atleast_1d(buffer_dict['PolarTargetPos']['mean'][1]),
            1.0 * torch.atleast_1d(buffer_dict['PolarTargetPos']['mean'][3] * torch.clamp(1.0 - torch.abs(buffer_dict['PolarTargetPos']['mean'][1]), min=0.0)),
        ]).pow(2).sum()
        loss_cov = 1e0 * torch.trace(buffer_dict['PolarTargetPos']['cov'])
        return {
            "mean": loss_mean,
            "cov": loss_cov
        }
