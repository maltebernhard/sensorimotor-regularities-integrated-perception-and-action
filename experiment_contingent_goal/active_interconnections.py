...
class Triangulation_AI_Decoupled(ActiveInterconnection):
    def __init__(self, estimators: List[RecursiveEstimator], device, object_name:str="Target") -> None:
        self.object_name = object_name
    ...
    def implicit_interconnection_model(self, meas_dict: Dict[str, torch.Tensor]):
        offset_angle = torch.tensor(meas_dict[f'Polar{self.object_name}Pos'][1].item(), device=self.device)
        ...
        angular_vel = torch.tensor(meas_dict[f'Polar{self.object_name}Pos'][3].item(), device=self.device) + torch.tensor(meas_dict['RobotVel'][2], device=self.device)
        # TODO: tan or plain?
...
class Triangulation_AI(ActiveInterconnection):
    ...
    def implicit_interconnection_model(self, meas_dict: Dict[str, torch.Tensor]):
        ...
        print(f"Angular Vel: {angular_vel.item()} = {meas_dict[f'Polar{self.object_name}Pos'][3].item()} + {meas_dict['RobotVel'][2].item()}")
