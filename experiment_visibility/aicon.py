...
class VisibilityAICON(AICON):
    ...
    def define_active_interconnections(self):
        AIs = {
            ...
            "Triangulation": Triangulation_Detached_AI(self.device),
            #"TargetVisibility": Visibility_Angle_AI(self.device, object_name="Target", sensor_angle_rad=self.env.robot.sensor_angle),
            "TargetVisibility": Visibility_Detached_AI(self.device, object_name="Target", sensor_angle_rad=self.env.robot.sensor_angle),
        }
        for i in range(1, self.num_obstacles + 1):
